...
test("Navigation_Bar", async ({ page }) => {
  ...
  await expect(
    page.getByRole("textbox", { name: "Ask me anything..." }),
  ).toBeVisible();
});
test("Progress_Profit_div", async ({ page }) => {
  await page.goto("http://localhost:8080/scorecard");
  await expect(
    page.locator("div").filter({ hasText: /^Profit$/ }),
  ).toBeVisible();
});
test("Progress_ProductProfitablity_div", async ({ page }) => {
  ...
  await expect(
    page
      .locator("div")
      .filter({ hasText: /^Product Profitability$/ })
      .first(),
  ).toBeVisible();
});
test("Progress_CostAnalysis_div", async ({ page }) => {
  await page.goto("http://localhost:8080/scorecard");
  await expect(
    page.locator("div").filter({ hasText: /^Cost Analysis$/ }),
  ).toBeVisible();
});
test("Progress_ComperativePerformance_div", async ({ page }) => {
  await page.goto("http://localhost:8080/scorecard");
  await expect(
    page.locator("div").filter({ hasText: /^Comparative Performance$/ }),
  ).toBeVisible();
});
...
test("Progress_Profit_ext", async ({ page }) => {
  ...
  await page
    ...
    .click();
  await expect(
    page.getByRole("dialog", { name: "Profit Breakdown" }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Profit Breakdown" }),
  ).toBeVisible();
  await expect(page.getByRole("heading")).toContainText("Profit Breakdown");
  await expect(page.getByLabel("Profit Breakdown")).toContainText(
    "Metricvs Last Monthvs Last Year",
  );
...
test("Progress_CostAnalysis_ext", async ({ page }) => {
  ...
  await page
    .locator("div")
    .filter({ hasText: /^Cost Analysis$/ })
    .getByRole("img")
    .click();
  await expect(
    page.getByRole("dialog", { name: "Understanding Costs" }),
  ).toBeVisible();
  await expect(page.locator("h2")).toContainText("Understanding Costs");
  await expect(page.getByLabel("Understanding Costs")).toContainText(
    "Variable Costs",
  );
  await expect(page.getByLabel("Understanding Costs")).toContainText(
    "These costs change in proportion to your business activity. Examples include raw materials, direct labor, and sales commissions. As your production or sales increase, these costs increase accordingly.",
  );
  await expect(page.getByLabel("Understanding Costs")).toContainText(
    "Fixed Costs",
  );
  await expect(page.getByLabel("Understanding Costs")).toContainText(
    "These costs remain constant regardless of your business activity level. Examples include rent, insurance, and salaries of permanent staff. They don't change with production or sales volume.",
  );
  await page.getByRole("button", { name: "Close" }).click();
  await expect(page.getByRole("navigation")).toMatchAriaSnapshot(`
      - navigation:
...
test("Progress_ComperativePerformance_ext", async ({ page }) => {
  ...
  await page
    .locator("div")
    .filter({ hasText: /^Comparative Performance$/ })
    .getByRole("button")
    .click();
  await expect(
    page.getByRole("dialog", { name: "Performance Benchmarking" }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("button", { name: "Talk to my advisor for more" }),
  ).toBeVisible();
  await page.getByRole("button", { name: "Close" }).click();
  await expect(page.getByRole("navigation")).toMatchAriaSnapshot(`
      - navigation:
