...
test('MainPage_Sales_div', async ({ page }) => {
  ...
  await expect(page.locator('div').filter({ hasText: /^Sales$/ })).toBeVisible();
  });
test('MainPage_ProductSales_div', async ({ page }) => {
  await page.goto('http://localhost:8080/');
  await expect(page.locator('div').filter({ hasText: /^Product Sales$/ })).toBeVisible();
  });
test('MainPage_WeekOverwiew_div', async ({ page }) => {
  await page.goto('http://localhost:8080/');
  await expect(page.locator('div').filter({ hasText: /^Week Overview$/ }).nth(1)).toBeVisible();
  });
  test('Sales_ext', async ({ page }) => {
    await page.goto('http://localhost:8080/');
    await page.locator('div').filter({ hasText: /^Sales$/ }).getByRole('button').click();
    await expect(page.getByRole('dialog', { name: 'Today\'s Salient Points' })).toBeVisible();  
  ...
  test('Week_Overwiew_ext', async ({ page }) => {
    ...
    await expect(page.getByRole('dialog', { name: 'This Week\'s Performance' })).toBeVisible();
