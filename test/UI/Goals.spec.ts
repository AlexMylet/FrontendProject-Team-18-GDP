...
test("Goals_div", async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Goals & Quests' })).toBeVisible();
    await expect(page.getByRole('button', { name: 'Add New' })).toBeVisible();
    await expect(page.locator('div').filter({ hasText: /^Goals$/ })).toBeVisible();
    await expect(page.locator('div').filter({ hasText: /^Quests$/ })).toBeVisible();
    await expect(page.locator('div').filter({ hasText: /^Leaderboards$/ })).toBeVisible();
    await expect(page.locator('div').filter({ hasText: /^Streaks$/ })).toBeVisible();
    await expect(page.locator('div').filter({ hasText: /^Awards$/ })).toBeVisible();
    await expect(page.getByText('The app has made you: £')).toBeVisible();
});
...
test("Goals_add", async ({ page }) => {
    ...
    await expect(page.locator('div').filter({ hasText: /^Target$/ }).locator('#target')).toBeVisible();
    ...
    await page.locator('div').filter({ hasText: /^Target$/ }).locator('#target').click();
    await page.locator('div').filter({ hasText: /^Target$/ }).locator('#target').fill('10');
...
test("Goals_edit", async ({ page }) => {    
    ...
    await page.locator('div').filter({ hasText: /^Target$/ }).locator('#target').click();
    await page.locator('div').filter({ hasText: /^Target$/ }).locator('#target').fill('20');
...
test("Leadrboard_mock_data", async ({ page }) => {   
    ...
    await expect(page.locator('#root')).toMatchAriaSnapshot(`- text: /1 Alice \\d+ units sold 2 Bob \\d+ units sold/`);

    await expect(page.getByText('Tracks the highest earnings')).toBeVisible();
    await expect(page.locator('#root')).toMatchAriaSnapshot(`- text: /1 Bob \\d+ GBP 2 Alice \\d+ GBP/`);
});
...
test("Awards_mock_data", async ({ page }) => {   
    await page.goto('http://localhost:8080/goals');
    await expect(page.locator('#root')).toMatchAriaSnapshot(`- text: "/Connect your data You connected your business data to platform Achieved: \\\\d+-\\\\d+-\\\\d+/"`);
    await expect(page.getByText('Achieved: 2025-04-11')).toBeVisible();
    await expect(page.locator('#root')).toMatchAriaSnapshot(`- text: "/View a leaderboard You viewed a leaderboard to compare against your peers Achieved: \\\\d+-\\\\d+-\\\\d+/"`);
